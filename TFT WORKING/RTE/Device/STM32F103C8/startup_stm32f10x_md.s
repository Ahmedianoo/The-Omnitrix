...
RTCAlarm_IRQHandler PROC					
				...
				LDR R0, =RTC_CRL               
				LDR R1, [R0]                   
				BIC R1, R1, #0x02              ; Clear ALRF (bit 1)
				STR R1, [R0]                   
				...
				LDR R1,[R0]
				;MOV R1,#1           ; Prepare bit mask for EXTI Line 17
				;LSL R1,#17
				;STR R1, [R0]                   ; Write to clear the pending flag
				
				MOV R1, #(1 << 17)               ; Bit mask for EXTI Line 17
				STR R1, [R0]                     ; Clear the pending flag
